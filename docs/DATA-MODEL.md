...
| `topic` | string | What the chapter covers, e.g. "Sequences and Series"; shown as the caption above the test list |
...
`student/src/lib/questionBank.ts` holds the seed bank: sixteen chapters - four per
subject - of six questions each, every one with a worked solution. `demoStore` turns each
chapter into one free topical test plus five locked ones, so the Practice Zone shows 96
tests of which 16 are playable end to end.

The demo also seeds twelve past attempts spread over the last fortnight (two today, one
yesterday, so the streak counter reads), three doubts of which two are answered, five
courses with fourteen lessons, twelve universities, six live and recorded courses, and a
twelve-person roster in the console. Attempts are generated by a fixed-seed pseudo-random
generator so the demo looks the same on every reload rather than reshuffling.

Because demo mode has no persistent accounts, `demoStore.attachDemoHistory(userId)` hands
the seeded attempts and doubts to whoever signs in - otherwise a first run would open on
empty analytics, which is the opposite of what a demo should show. None of this exists in
live mode: there, the console's Test bank writes the real content.
